...
from schemas import CreateInterviewResponse, CreateInterviewRequest, GetInterviewRequest, GetInterviewResponse
...
async def get_interview(request: GetInterviewRequest):
    ...
    try:
        interview = (db.collection("users").document(request.userId)
